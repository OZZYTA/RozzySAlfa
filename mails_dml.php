...
function mails_insert(&$error_message = '') {
	global $Translation;
...
function mails_copy_children($destination_id, $source_id) {
	global $Translation;
...
function mails_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	...
	if(!check_record_permission('mails', $selected_id, 'delete')) {
...
function mails_update(&$selected_id, &$error_message = '') {
	global $Translation;

	// mm: can member edit record?
	if(!check_record_permission('mails', $selected_id, 'edit')) return false;
...
function mails_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = '') {
	...
	// process form title
	$templateCode = str_replace('<%%DETAIL_VIEW_TITLE%%>', 'Detalle de Correo Enviado', $templateCode);
	...
	$templateCode = str_replace('<%%UPLOADFILE(cuerpo)%%>', '', $templateCode);
	...
	if($selected_id) {
		...
		$templateCode = str_replace('<%%VALUE(idpqr)%%>', safe_html($urow['idpqr']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(idpqr)%%>', urlencode($urow['idpqr']), $templateCode);
		$templateCode = str_replace('<%%VALUE(fecha)%%>', safe_html($urow['fecha']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(fecha)%%>', urlencode($urow['fecha']), $templateCode);
		$templateCode = str_replace('<%%VALUE(eps)%%>', safe_html($urow['eps']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(eps)%%>', urlencode($urow['eps']), $templateCode);
		$templateCode = str_replace('<%%VALUE(correo)%%>', safe_html($urow['correo']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(correo)%%>', urlencode($urow['correo']), $templateCode);
		$templateCode = str_replace('<%%VALUE(asunto)%%>', safe_html($urow['asunto']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(asunto)%%>', urlencode($urow['asunto']), $templateCode);
		$templateCode = str_replace('<%%VALUE(cuerpo)%%>', safe_html($urow['cuerpo']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(cuerpo)%%>', urlencode($urow['cuerpo']), $templateCode);
		...
		$templateCode = str_replace('<%%VALUE(groupID)%%>', safe_html($urow['groupID']), $templateCode);
		...
		$templateCode = str_replace('<%%VALUE(cuerpo)%%>', '', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(cuerpo)%%>', urlencode(''), $templateCode);
